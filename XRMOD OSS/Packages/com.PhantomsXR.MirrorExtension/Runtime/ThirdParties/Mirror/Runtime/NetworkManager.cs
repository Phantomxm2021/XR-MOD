...
//            NetworkServer.AddPlayerForConnection(conn, player);
            ...
            if (startPos != null)
            {
                tmp_NotificationData.Prefab = playerPrefab;
                tmp_NotificationData.Position = startPos.position;
                tmp_NotificationData.Rotation = startPos.rotation;
            }
            else
            {
                tmp_NotificationData.Prefab = playerPrefab;
            ...
            if (tmp_Objects != null && tmp_Objects.Count > 0)
            {
                GameObject player = (GameObject) tmp_Objects[0];
                NetworkServer.AddPlayerForConnection(conn, player);

                // instantiating a "Player" prefab gives it the name "Player(clone)"
                // => appending the connectionId is WAY more useful for debugging!
                player.name = $"{player.name.Replace("(Clone)", String.Empty)}[netId={conn.identity.netId}]";
            }
        }
