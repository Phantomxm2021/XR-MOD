    ...
    public static class NetworkClient
    {
        ...
        static NetworkIdentity SpawnPrefab(SpawnMessage message)
        {
            if (GetPrefab(message.assetId, out GameObject prefab))
            {
                ...
                GameObject tmp_PrefabInstanced = null;
                //For ILRT
                try
                {
                    var tmp_NotificationData = new InstantiateNotificationData()
                        {Prefab = prefab, Position = message.position, Rotation = message.rotation};

                    List<object> tmp_Objects =
                    ...
                    tmp_PrefabInstanced = tmp_Objects[0] as GameObject;
                    tmp_PrefabInstanced.name = tmp_PrefabInstanced.name.Replace("(Clone)", string.Empty);
                    tmp_PrefabInstanced.name = $"{tmp_PrefabInstanced.name}[netId={message.netId}]";
                }
                catch (Exception tmp_Exception)
                {
                    Debug.LogError(tmp_Exception.Message);
                    tmp_PrefabInstanced = Object.Instantiate(prefab, message.position, message.rotation);
                    tmp_PrefabInstanced.name = $"{tmp_PrefabInstanced.name}[connId={connection.connectionId}]";
                }

                //GameObject obj = GameObject.Instantiate(prefab, message.position, message.rotation);
                //Debug.Log("Client spawn handler instantiating [netId:" + msg.netId + " asset ID:" + msg.assetId + " pos:" + msg.position + " rotation: " + msg.rotation + "]");
                return tmp_PrefabInstanced.GetComponent<NetworkIdentity>();
            }
        ...
        static NetworkIdentity SpawnSceneObject(SpawnMessage message)
        {
            ...
            {
                Debug.LogError(
                    $"Spawn scene object not found for {message.sceneId:X}. Make sure that client and server use exactly the same project. This only happens if the hierarchy gets out of sync.");
        ...
        public static void PrepareToSpawnSceneObjects()
        {
            ...
            foreach (NetworkIdentity identity in allIdentities)
            {
                ...
                {
                    foreach (KeyValuePair<ulong, NetworkIdentity> tmp_Identity in spawnableObjects)
                    {
                        Debug.Log(tmp_Identity.Value.gameObject.name);
                    }

                    spawnableObjects.TryAdd(identity.sceneId, identity);
                }
            }
        }
...
#pragma warning restore 618
        ...
        internal static void OnHostClientSpawn(SpawnMessage message)
        {
            ...
            if (NetworkServer.spawned.TryGetValue(message.netId, out NetworkIdentity localObject) &&
                localObject != null)
            {
